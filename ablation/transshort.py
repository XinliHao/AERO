...
class TransShort(nn.Module):
    def __init__(self, embed_time, slide_win, small_win, dim):
        super(TransShort, self).__init__()
    ...
    def forward(self, src, src_time):
        src = self.Linear1(src)
        src_time_enc, inter = self.time_bias_encoder(src, src_time)
        src = src + src_time_enc
        memory = self.transformer_encoder(src).float()
        memory = self.Linear3(memory)
        x = self.fcn(memory).float()
        return x